...
$scatid = isset($_GET['scat']) ? intval($_GET['scat']) : null;
$catid = isset($_GET['cat']) ? intval($_GET['cat']) : null;
$pdtobj = new MysqliDb(HOST, USER, PWD, DB);
if($catid){
$pdtobj->where("pm.ct_id", $catid);
}
if($scatid){
$pdtobj->where("pm.sct_id", $scatid);
}
...
<div class="tf-page-title" style="background-color: #fbae16;background-image: url('<?php echo ROOT?>images/bg.png');background-size: cover;background-repeat: no-repeat;">
    ...
    <div class="heading text-center">
        <?php 
            if($_GET['cat'] == 1)      echo "Men's Collections";
            elseif($_GET['cat'] == 2)  echo "Women's Collections";
            elseif($_GET['cat'] == 3)  echo "Accessories";
        ?>
    </div>
<?php }else{ ?>
        <div class="heading text-center"><?php echo ($scatid && !empty($pdtarray))? "Collections of ".$pdtarray[0]["sct_title"]:"All Collections" ?></div>
